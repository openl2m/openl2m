...
PDFs
----
The public PDF "H3C Access Controllers REST API Developers Guide-6W100-book" describes basic REST access, and
has examples of Python code.

The HPE support site (https://https://networkingsupport.hpe.com/ at time of writing) has the login-required PDF on the Networking suppport site
"HPE FlexFabric 5944&5945 Comware 7 API Reference", where all the current API endpoints are described.
...
Specifically, the driver inherits the RestConnector() that implements all the HTTP calls.
See *switches/connect/restconnector.py*.

HTTP GET is used to read various API end-points, documented in the NetConf docs described above.
These calls return body data with JSON structures related to interfaces, PoE, etc.
We use those as regular Python Dictionaries.
...
In most cases, this is just "IfIndex". And, e.g. for Poe Port status, it includes the PSEID,
ie the power supply that services a specific port!
