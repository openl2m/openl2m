...
if sys.version_info < (3, 8):
    raise RuntimeError(
        "OpenL2M requires Python 3.8 or higher (current: Python {})".format(
            sys.version.split()[0]
        )
    )
...
HIDE_NONE_ETHERNET_INTERFACES = getattr(
    configuration, "HIDE_NONE_ETHERNET_INTERFACES", False
)
...
IFACE_ALIAS_KEEP_BEGINNING_REGEX = getattr(
    configuration, "IFACE_ALIAS_KEEP_BEGINNING_REGEX", ""
)
...
SNMP_TIMEOUT = getattr(
    configuration, "SNMP_TIMEOUT", 4
)  # seconds before retry, see EasySNMP docs
SNMP_RETRIES = getattr(configuration, "SNMP_RETRIES", 3)  # retries before fail
SNMP_MAX_REPETITIONS = getattr(
    configuration, "SNMP_MAX_REPETITIONS", 10
)  # SNMP get_bulk max_repetitions
...
DATA_UPLOAD_MAX_NUMBER_FIELDS = getattr(
    configuration, "DATA_UPLOAD_MAX_NUMBER_FIELDS", 10000
)
...
if LDAP_CONFIG is not None:
    ...
    AUTH_LDAP_GROUP_TO_SWITCHGROUP_REGEX = getattr(
        LDAP_CONFIG, "AUTH_LDAP_GROUP_TO_SWITCHGROUP_REGEX", None
    )
...
EMAIL_SUBJECT_PREFIX = getattr(
    configuration, "EMAIL_SUBJECT_PREFIX", "[OpenL2M-Admin] "
)
EMAIL_SUBJECT_PREFIX_USER = getattr(
    configuration, "EMAIL_SUBJECT_PREFIX_USER", "[OpenL2M] "
)
