...
class ApiBasivView(TemplateView):
...
class InterfaceArpView(APIView):
    ...
    """
    def get(self, request, group_id, switch_id, interface_name='',):
        group = get_object_or_404(SwitchGroup, pk=group_id)
        switch = get_object_or_404(Switch, pk=switch_id)
        if not rights_to_group_and_switch(request=request, group_id=group_id, switch_id=switch_id):
            error = Error()
