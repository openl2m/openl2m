...
class SnmpConnectorJuniper(SnmpConnector):
    ...
    def _get_vlan_data(self):
        ...
        retval = self.get_branch_by_name('jnxL2aldVlanTag', True, self._parse_mibs_juniper_l2ald_vlans)
        if retval < 0:  # error
            return retval
        if retval > 0:  # we found something, read the rest
            retval = self.get_branch_by_name('jnxL2aldVlanName', True, self._parse_mibs_juniper_l2ald_vlans)
            if retval < 0:  # error
                return retval
            retval = self.get_branch_by_name('jnxL2aldVlanType', True, self._parse_mibs_juniper_l2ald_vlans)
            if retval < 0:  # error
                return retval
            retval = self.get_branch_by_name('jnxL2aldVlanFdbId', True, self._parse_mibs_juniper_l2ald_vlans)
            if retval < 0:  # error
                return retval
            return 1
        return 0
